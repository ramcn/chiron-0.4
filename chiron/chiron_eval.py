        ...
        for name in tqdm(file_list, desc="CTC decoding.",position = 2):
            ...
            with tqdm(total=reads_n, desc="ctc decoding",position = 3) as pbar:
                while True:
                    ...
                    l_sz, d_sz = sess.run([logits_queue_size, decode_queue_size],options=options, run_metadata=run_metadata)
                    ...
                    decoded_fname, i, predict_val, logits_prob = sess.run(decode_ops, feed_dict={training: False},options=options, run_metadata=run_metadata)
